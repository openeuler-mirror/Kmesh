...
static int handle_tcp_timeout(struct bpf_sock_ops *skops)
{
    int error_count_key = skops->remote_ip4; // access the map of break count
    int break_key = 1;
    int is_break_key = skops->remote_ip4; 
    int *error_count, *limit, current_error_count = 1;
    int break_val = 1;

    error_count = bpf_map_lookup_elem(&map_of_break_count, &error_count_key);
    if (error_count) {
        current_error_count += *error_count;
    }
    bpf_map_update_elem(&map_of_break_count, &error_count_key, &current_error_count, BPF_ANY);

    limit = bpf_map_lookup_elem(&map_of_break, &break_key);
    if (limit && current_error_count > *limit) {
        bpf_map_update_elem(&map_of_is_break, &is_break_key, &break_val, BPF_ANY);
		return 0;
    }
    return 0;
}
